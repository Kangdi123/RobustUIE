...
if __name__ == '__main__':
    ...
    for task_type in args.run_tasks:
        ...
        golds = [example['conversations'][-1]['value'] for example in test_data]
        outputs = inference(llm, test_data)
        output_ans[task_type]=[]
        for pred,gold,example in zip(outputs,golds,test_data):
            case1=example
            case1['pred']=pred
            case1['gold']=gold
            output_ans[task_type].append(case1)
...
#
        ...
        if os.path.exists(mid_file):
            ...
            cur_data = read_json_file(test_task_files[task_type])

        assert len(cur_data) == len(output_ans[task_type]), '运行样本数应该相同'

        for idx in tqdm(range(len(cur_data))):
            for setting in query_settings:
                cur_data[idx][args.prediction_name + '-' + setting] = output_ans[task_type][idx]['pred']
