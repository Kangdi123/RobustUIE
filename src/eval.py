...
def eval_res(args):
    ...
    if args.task_type == "EAE":
        ...
        source_dict = gen_idx_sources_dict(path=args.input_file) if args.granularity == "source" or args.granularity == "source_type" else {}
        prf_s = cal_scores_ai_ac(pred_dict, gold_dict, args.match_type, args.granularity, source_dict)
        # {'AI': 'Argument Identification 预测论元内容/边界', 'AC': 'Argument Classification 预测论元角色'}
        ...
        # {'TI': 'Trigger Identification 预测触发词内容/边界', 'TC':'Trigger Classification 预测事件类型',
        #  'AI': 'Argument Identification 预测论元内容/边界', 'AC': 'Argument Classification 预测论元角色'}
        metric_names = ['TI', 'TC', 'AI', 'AC']
    elif args.task_type == "ED":
        if args.eval_type == 'obj':
            pred_list, gold_list = cvt_ee.do_convert(args)
            pred_dict = gen_idx_events_dict(events_list=pred_list)
            gold_dict = gen_idx_events_dict(events_list=gold_list)
        ...
        # {'EI': 'Entity Identification 预测实体内容/边界', 'EC': 'Entity Classification 预测实体类型'}
        metric_names = ['EI', 'EC']
        ...
        # {'RI': 'Relation Identification 预测头/尾实体', 'RC': 'Relation Classification 预测关系类型'}
        metric_names = ['RI', 'RC']
        ...
        for source, prfs in prf_s.items():
            ...
            for i, prf in enumerate(prfs):
                try:
                    ...
                    pass
    elif args.granularity == 'source_type':
        for type, prfs in prf_s.items():
            result[type] = {}
            for source, prf1 in prfs.items():
                result[type][source]= {}
                for i, prf in enumerate(prf1):
                    
                    result[type][source][f'{args.match_type}-{metric_names[i]}-P'] = f"{prf[0]*100:.1f}"
                    result[type][source][f'{args.match_type}-{metric_names[i]}-R'] = f"{prf[1]*100:.1f}"
                    result[type][source][f'{args.match_type}-{metric_names[i]}-F1'] = f"{prf[2]*100:.1f}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task_type", type=str, default="EAE", choices=['EAE', 'EE', 'NER', 'RE','ED'])
    ...
    parser.add_argument("--granularity", type=str, default="overall", choices=['overall', 'type', 'source',"source_type"])
